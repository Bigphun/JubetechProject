...
export const CategoryProvider = ({
  ...
  const fetchAllCategories = async (message: string = "") => {
    try {
      ...
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}/category/all`
      );
      dispatch({
        type: "FETCH_SUCCESS",
  ...
  const fetchPaginationCategories = async (page: number, pageSize: number) => {
    try {
      dispatch({ type: "FETCH_START" });
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}/category/pagination`,
        {
      ...
      dispatch({
        ...
        message: "Categories were queried successfully.",
        status: response.status,
      });
    } catch (error) {
      if (axios.isAxiosError(error)) {
  ...
  const fetchcategoryById = async (category_id: string) => {
    try {
      dispatch({ type: "FETCH_START" });
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}/category/id/${category_id}`
      );
      dispatch({
        type: "FETCH_SUCCESS",
        payload: response.data.data,
        message: "The category was queried successfully.",
        status: response.status,
      });
    } catch (error) {
      if (axios.isAxiosError(error)) {
  ...
  const searchCategories = async (searchParam: IFSearchParam) => {
    try {
      dispatch({ type: "FETCH_START" });
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}/category/search`,
        {
          params: { ...searchParam },
        }
      );
      dispatch({
        type: "FETCH_SUCCESS",
        payload: response.data.data,
        message: "Categories were searched successfully",
        status: response.status,
      });
    } catch (error) {
      if (axios.isAxiosError(error)) {
